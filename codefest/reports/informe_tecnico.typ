#set page(paper: "a4", margin: (x: 2cm, y: 1.8cm), numbering: "1 / 1")
...
El sistema es un recuperador denso multi-encoder sobre 1.826 documentos
...
#table(
  ...
  fill: (_, y) => if y == 0 { rgb("#f2f2f2") },
  [*Fenómeno*], [*Documentos*], [*Fragmentos aprox.*],
    [F1 — IA y capacidades estratégicas], [459], [—],
    [F2 — Seguridad del entorno espacial], [479], [—],
    [F3 — Dinámicas territoriales], [888], [—],
)
...
(por ejemplo `F1-AIINDEX-001`); se verificó que las 1.826 entradas del
...
#table(
  ...
  [*Formato*], [*Documentos*], [*Tratamiento*],
    [PDF], [759], [PyMuPDF en orden de lectura; OCR si no hay capa de texto],
    [JSON], [954], [adaptador por observatorio; campos descriptivos a metadata],
    [CSV], [26], [cada fila como pares columna: valor],
    [XLSX], [4], [igual que CSV, hoja a hoja],
    [PBF], [73], [atributos del tile, deduplicados entre niveles de zoom],
    [Imagen], [9], [OCR (spa+eng+por)],
    [TXT], [1], [texto plano],
)
...
se recuperaron *886 páginas* de 144 documentos,
sobre 36.828 páginas de PDF procesadas.
...
y portugués: 1022 documentos en inglés,
697 en español y 75 en portugués.
...
títulos dudosos sobre 1.826 documentos.
...
empaquetan de forma codiciosa hasta 190 palabras, con
tope duro en 240 y solapamiento de una oración entre
...
+ *Cumplimiento del requisito de completitud lingüística (Sección 3.3).* Los
  ...
  después por coma. Afecta a 3.495 fragmentos de
  91.088 (3.8%) y se
  contabiliza aparte.
...
+ *El tope de 240 palabras está por debajo del límite de
  250 de la salida.* Así ningún fragmento recuperado necesita
...
+ *El buffer persiste entre párrafos de una misma sección.* Un párrafo suelto de
  ...
  de 96 a 198 palabras y el número de fragmentos bajó un 25%.

Resultado: *91.088 fragmentos*, media 185 palabras,
mediana 198, percentil 90 224, máximo
240. Ningún documento se queda sin fragmentos: los archivos sin
...
#table(
  columns: (1.6fr, 1.2fr, auto, auto, auto),
  inset: 5pt,
  stroke: 0.4pt + rgb("#cccccc"),
  fill: (_, y) => if y == 0 { rgb("#f2f2f2") },
  [*Encoder*], [*Arquitectura*], [*Licencia*], [*Dim.*], [*Índice*],
    [`granite-embedding-311m-multilingual-r2`], [ModernBERT (encoder-only)], [Apache 2.0], [768], [280 MB],
    [`bge-m3`], [XLM-RoBERTa (encoder)], [MIT], [1024], [373 MB],
    [`multilingual-e5-large`], [XLM-RoBERTa (encoder)], [MIT], [1024], [373 MB],
)
...
aproximación. Con 91.088 vectores la búsqueda exhaustiva tarda
...
El grafo tiene *33.178 entidades* y *331.721 relaciones*, con
...
Tipos de entidad: organizacion (16248), persona (6681), otro (5480), lugar (2907), marco_juridico (831), municipio (654).
...
lema del verbo es el tipo de relación. Las relaciones más frecuentes son `announced` (610), `create` (468), `stated` (413), `posted` (397), `said` (265).
...
cada índice devuelve sus 200 candidatos más similares, y las
listas se combinan. Después:

*Fusión (Sección 8.4).* Se usa *Reciprocal Rank Fusion* con
#emph[k#sub[0]] = 60. Frente a CombSUM y CombMNZ, RRF combina rangos
...
*Tope de diversidad.* Como máximo 3 fragmentos por
...
> 0,98) o si es una traducción (coseno
> 0,95 y en otro idioma). Una edición posterior en
...
sección— se rellena el presupuesto de 250 palabras con los
...
#table(
  ...
  [*Configuración*], [*F1\@3*], [*Cobertura*], [*Known-item*], [*Títulos*], [*Duplic.*],
    [solo granite], [0,042], [0,450], [0,808], [0,842], [0,04],
    [solo bge_m3], [0,175], [0,444], [0,850], [0,917], [0,28],
    [solo e5_large], [0,050], [0,319], [0,800], [0,925], [0,08],
    [fusión rrf], [0,092], [0,478], [0,850], [0,917], [0,30],
    [fusión combsum], [0,092], [0,454], [0,850], [0,917], [0,32],
    [fusión combmnz], [0,092], [0,435], [0,850], [0,917], [0,32],
    [rrf + prior fenómeno], [0,092], [0,478], [0,850], [0,917], [0,30],
    [rrf sin expansión de vecinos], [0,092], [0,471], [0,850], [0,917], [0,30],
    [rrf, expansión a 200 palabras], [0,092], [0,474], [0,850], [0,917], [0,30],
    [rrf, expansión a 225 palabras], [0,092], [0,475], [0,850], [0,917], [0,30],
    [rrf, expansión a 245 palabras], [0,092], [0,478], [0,850], [0,917], [0,30],
    [rrf, expansión a 250 palabras], [0,092], [0,479], [0,850], [0,917], [0,30],
    [sin poda de duplicados], [0,092], [0,478], [0,850], [0,917], [0,36],
    [poda duplicados cos>0.98], [0,092], [0,478], [0,850], [0,917], [0,30],
    [poda duplicados cos>0.95], [0,092], [0,478], [0,850], [0,917], [0,28],
    [poda duplicados cos>0.92], [0,092], [0,478], [0,850], [0,917], [0,00],
    [rrf, cola doc peso 0.0 n=0], [0,092], [0,478], [0,850], [0,917], [0,30],
    [rrf, cola doc peso 0.05 n=4], [0,092], [0,478], [0,850], [0,917], [0,38],
    [rrf, cola doc peso 0.15 n=4], [0,092], [0,478], [0,850], [0,917], [0,38],
    [rrf, cola doc peso 0.35 n=4], [0,042], [0,478], [0,850], [0,917], [0,36],
    [rrf, máx 2 frag/doc], [0,092], [0,478], [0,850], [0,917], [0,30],
    [rrf, máx 3 frag/doc], [0,092], [0,478], [0,850], [0,917], [0,30],
    [rrf, máx 4 frag/doc], [0,092], [0,478], [0,850], [0,917], [0,30],
    [rrf, máx 10 frag/doc], [0,092], [0,478], [0,850], [0,917], [0,30],
    [rrf + grafo], [0,092], [0,478], [0,833], [0,917], [0,30],
    [rrf + grafo + prior], [0,092], [0,478], [0,833], [0,917], [0,30],
)
...
Conviene decir qué costó: el grafo pasó de 35.022 a
33.178 entidades y de 346.487 a
331.721 relaciones. Parte de esa diferencia no es pérdida sino
